...
function getTimerWrapperSourcePath() {
  return '@agoric/vat-timerWrapper';
}
...
async function buildSwingset(
  ...
  const mkRd = resName => ({
    ...
    async bundleSource() {
      ...
      const src = String.fromArrayBuffer(rsrc.slice(0));
      console.log('@@bundleSource got:', resName, src.slice(0, 60) + '...');
      return { source: src, sourceMap: null };
    }
  });
  config.devices = [
...
export default async function start(basedir, withSES, argv,
  ...
  console.log('@@await buildSwingset done');
