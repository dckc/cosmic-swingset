  ...
  return helpers.makeLiveSlots(
    ...
    (E, D) => {
      async function createPrivateBundle(vats) {
        return harden({
          pledger: await E(vats.pledger).getPledger(), // linked to registry in vat-http.js
      ...
      function makeBundler(contractHost, vats) {
        return harden({
          async createDemoBundle(nickname) {
            ...
            // Register some assays.
            await E(pixelBundle.gallery).getAssays().then(assays =>
              Promise.all(['pixelAssay', 'dustAssay'].map(name =>
                E(registry).set(name, assays[name]))));
      ...
      return harden({
        async bootstrap(argv, vats, devices) {
          ...
          if (ROLES.client || ROLES.one_client) {
            ...
            await E(vats.http).setPresences(bundle, await createPrivateBundle(vats));
