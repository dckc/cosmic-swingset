...
function build(E, D) {
  ...
  return {
          ...
          },
          {
            CTP_OPEN(obj) {
              ...
              const { dispatch } = makeCapTP(obj.connectionID, sendObj, () => {
                // Harden our exported objects.
                harden(exportedToCapTP);
                if (obj.url === '/captp') {
                  return exportedToCapTP;
                } else if (obj.url === '/captp/pledger') {
                  // Pledger endpoint only gets the userFacet.
                  return harden({
                    getPledger(name) {
                      return E(homeObjects.pledger).makeUserFacet(name);
                    },
                    wait() {
                      return readyForClient.p;
                    },
    ...
    setPresences(ps, privateObjects) {
      ...
      if (privateObjects.pledger && ps.registry) {
        E(privateObjects.pledger).setRegistry(ps.registry);
      }
